...
test.describe("product form create page ui and functional regression", () => {
  test("functional: all interactive elements are operable and form submit creates product", async ({ page, request }) => {
    ...
    await page.getByTestId("product-form-number").fill(productNumber);
    await page.getByTestId("product-form-name").fill(productName);
    await page.getByTestId("product-form-description").fill(productDescription);
    await page.getByTestId("product-form-group").selectOption({ label: groupName });
    ...
    await expect(page.getByTestId("product-form-number")).toHaveValue(productNumber);
    await expect(page.getByTestId("product-form-number")).toBeDisabled();
    await expect(page.getByTestId("product-form-name")).toHaveValue(productName);
    await expect(page.getByTestId("product-form-description")).toHaveValue(productDescription);
    await expect(page.getByTestId("product-form-group")).toHaveValue(/\d+/);
    await expect(page.getByTestId("product-form-status")).toHaveValue("blocked");
