  ...
  return (
    <section className="page flex flex-col gap-6" data-testid="picking-page">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Left Sidebar: Pick Waves List */}
        <aside className="lg:col-span-4 xl:col-span-3 bg-[var(--panel)] rounded-[var(--radius-lg)] shadow-sm border border-[var(--line)] overflow-hidden flex flex-col h-full max-h-[calc(100vh-200px)]">
            ...
            <>
              ...
              <div className="bg-[var(--panel)] rounded-[var(--radius-lg)] shadow-sm border border-[var(--line)] p-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                  <div>
                    ...
                    <p className="text-sm text-[var(--muted)] mt-1">
                      ID: {selectedWave.id} • Erstellt am {new Date().toLocaleDateString()}
                    </p>
                  </div>
                ...
                {["released", "in_progress"].includes(selectedWave.status) && (
                  <div className="bg-blue-50/50 dark:bg-blue-900/10 rounded-[var(--radius-lg)] border border-blue-100 dark:border-blue-900/30 p-4 sm:p-6 mb-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="bg-[var(--panel)] rounded-[var(--radius-sm)] border border-[var(--line)] p-4 flex flex-col justify-center">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <span className="block text-xs font-medium text-[var(--muted)] uppercase tracking-wider mb-1">
                              Status
                            </span>
                            <p className="text-sm font-medium text-[var(--ink)]" data-testid="pick-scan-status">
                              {scanStatus}
                            </p>
                            {scanTask && (
                    ...
                    <tbody className="bg-[var(--panel)] divide-y divide-[var(--line)]">
                      {(waveDetailQuery.data?.tasks ?? []).map((task) => (
                        <tr key={task.id} className="hover:bg-[var(--panel-soft)] transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[var(--ink)]">
                      ...
                      {(waveDetailQuery.data?.tasks ?? []).length === 0 && (
                        <tr>
                          <td colSpan={6} className="px-6 py-8 text-center text-sm text-[var(--muted)] italic">
                            Keine Aufgaben in dieser Wave.
                          </td>
                        </tr>
                      )}
                    </tbody>
