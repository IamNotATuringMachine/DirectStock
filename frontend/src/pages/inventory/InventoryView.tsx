import { Activity, ArrowRightLeft, ChevronLeft, ChevronRight, Filter, Layers, MapPin, Package, Search, X } from "lucide-react";
import type { InventoryByProductItem, InventoryItem, LowStockItem, MovementItem, Warehouse } from "../../types";
...
type InventoryViewProps = {
  summary: InventorySummaryView | null;
  searchInput: string;
  ...
  onCloseDetail: () => void;
  detailStock: InventoryByProductItem[];
  detailStockLoading: boolean;
  detailMovements: MovementItem[];
  detailMovementsLoading: boolean;
};
  ...
  return (
    <section className="page flex flex-col gap-6" data-testid="inventory-page">
      ...
      <div className="kpi-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="kpi-card bg-white dark:bg-zinc-800 p-6 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm" data-testid="inventory-kpi-card">
          <div>
            <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Produkte mit Bestand</span>
            <strong className="text-3xl font-bold text-zinc-900 dark:text-zinc-100 mt-2 block">{summary?.total_products_with_stock ?? "-"}</strong>
          </div>
        </div>
        <div className="kpi-card bg-white dark:bg-zinc-800 p-6 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm" data-testid="inventory-kpi-card">
          <div>
            <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Gesamtbestand</span>
            <strong className="text-3xl font-bold text-zinc-900 dark:text-zinc-100 mt-2 block">{summary?.total_quantity ?? "-"}</strong>
          </div>
        </div>
        <div className="kpi-card bg-white dark:bg-zinc-800 p-6 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm" data-testid="inventory-kpi-card">
          <div>
            <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Reserviert</span>
            <strong className="text-3xl font-bold text-zinc-900 dark:text-zinc-100 mt-2 block">{summary?.reserved_quantity ?? "-"}</strong>
          </div>
        </div>
        <div className="kpi-card bg-white dark:bg-zinc-800 p-6 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm" data-testid="inventory-kpi-card">
          <div>
            <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Unter Meldebestand</span>
            <strong className="text-3xl font-bold text-rose-600 dark:text-rose-400 mt-2 block">{summary?.low_stock_count ?? "-"}</strong>
          </div>
        </div>
      </div>
            ...
            {lowStock.map((item) => (
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300">Kritisch</span>
                </div>
                <div className="flex items-center gap-2 mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                  <MapPin className="w-3 h-3" />
                  <span>{item.warehouse_code}</span>
                  <span className="text-zinc-300 dark:text-zinc-700">|</span>
                  <span className="font-medium text-rose-600 dark:text-rose-400">{item.on_hand} / {item.threshold}</span>
                </div>
              </div>
            ))}
            {lowStock.length === 0 ? <p className="text-sm text-zinc-500 dark:text-zinc-400 text-center py-4">Keine kritischen Bestände.</p> : null}
          </div>
        </article>
        <article className="subpanel bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm p-5 h-full">
      ...
      {selectedProduct ? (
        <div className="modal-backdrop fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onCloseDetail}>
          <div
          ...
          >
            <div className="panel-header p-6 border-b border-zinc-200 dark:border-zinc-700 flex items-start justify-between bg-zinc-50 dark:bg-zinc-900/50">
              ...
              <button className="btn p-2 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-colors text-zinc-500 dark:text-zinc-400" onClick={onCloseDetail} aria-label="Schließen">
                <X className="w-5 h-5" />
              ...
              <div className="two-col-grid grid grid-cols-1 md:grid-cols-2 gap-6">
                <article className="subpanel space-y-4">
                  ...
                  <div className="list-stack small space-y-2">
                    {detailStock.map((item) => (
                      <div key={item.inventory_id} className="list-item static-item p-3 rounded-lg bg-zinc-50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-700">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <strong className="text-sm text-zinc-900 dark:text-zinc-100">{item.warehouse_code} / {item.zone_code} / {item.bin_code}</strong>
                        </div>
                    ...
                    {!detailStockLoading && detailStock.length === 0 ? <p className="text-sm text-zinc-500 dark:text-zinc-400 italic">Kein Bestand vorhanden.</p> : null}
                  </div>
                </article>
                <article className="subpanel space-y-4">
                  <div className="flex items-center gap-2 border-b border-zinc-200 dark:border-zinc-700 pb-2">
                  ...
                  <div className="list-stack small space-y-2">
                    {detailMovements.map((movement) => (
                      <div key={movement.id} className="list-item static-item p-3 rounded-lg bg-zinc-50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-700">
                        <strong className="text-xs font-mono text-zinc-500 dark:text-zinc-400 block mb-1">{movement.reference_number ?? "-"}</strong>
                    ...
                    {!detailMovementsLoading && detailMovements.length === 0 ? <p className="text-sm text-zinc-500 dark:text-zinc-400 italic">Keine Bewegungen vorhanden.</p> : null}
                  </div>
                </article>
              </div>
            </div>
